...
const DBOperations = require(path.join(__dirname, 'modules/DBLayer/DBWithQuery'));
...
var db = DBOperations.OpenDB(__dirname + '/modules/DBLayer/SampleDB.db');
...
app.post('/api/Create', (req, res) => {
	try {
		DBOperations.CreateNewRecord(db, DBOperations.insert, [req.body.Name, req.body.Phone, req.body.Email, req.body.Role, req.body.Password]);
		res.json({ status: 'success' })
	}
	catch {
...
app.post('/api/CheckMailExists', function (req, res) {
	DBOperations.CheckMailExists(db, DBOperations.Check_Mail_Exists, [req.body.Email])
		.then(function (data) {
			res.json(data)
		})
		.catch(function (err) {
